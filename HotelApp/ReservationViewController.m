...
- (NSArray*)roomCategoriesFromPlistNamed:(NSString*)plistName {
    ...
    [newMenus addObjectsFromArray:dictionary[@"roomCategories"]];
    return newMenus;
}
...
- (void)viewDidLoad
{
    ...
    if (keyboardToolbar == nil) {
        ...
        [keyboardToolbar setItems:@[previousButton, nextButton, spaceBetweenButtons, doneButton] ];
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (index == 2) {
        ...
        return arrayRoomCategory[row][@"categoryName"];
    }
            ...
            if (![textField.text isEqualToString:@""] && i > 2) {
                for (NSDictionary *dic in arrayRoomCategory) {
                    ...
                    if ([categoryName isEqualToString:textField.text]) {
                        NSString *categoryPrice = dic[@"categoryPrice"];
                        NSDictionary *room = @{@"categoryName": categoryName,@"categoryPrice": categoryPrice};
...
- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([[segue identifier] isEqualToString:@"ReservationDetailSegue"]) {
        ...
        NSDictionary *reservationInfo = @{@"totalPrice": totalPriceString,@"arrivalDate": arrivalDate,@"departureDate": departureDate,@"numberOfPeople": numberOfPeople,@"roomCategory": roomCategory};
